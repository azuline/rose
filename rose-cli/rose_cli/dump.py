...
def release_to_json(r: Release) -> dict[str, Any]:
    return {
        ...
        "all_genres": uniq(r.genres + r.parent_genres + r.secondary_genres + r.parent_secondary_genres),
...
def track_to_json(t: Track, with_release_info: bool = True) -> dict[str, Any]:
    ...
    if with_release_info:
        r.update({
            ...
            "new": t.release.new,
            "favorite": t.release.favorite,
            "rating": t.release.rating,
...
def dump_all_tracks(c: Config, matcher: Matcher | None = None) -> str:
