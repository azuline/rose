            ...
            if stem == old_name {
                ...
                if extension != "toml" {
                    let new_adjacent_file =
                        collages_dir.join(format!("{}.{}", new_name, extension));
...
pub fn remove_release_from_collage(
    ...
    let release_logtext = get_release_logtext(config, release_id)?.ok_or_else(|| {
    ...
    if old_len == data.releases.len() {
        ...
        info!(
            "No-Op: Release {} not in collage {}",
            release_logtext, collage_name
        );
        return Ok(());
    }
...
pub fn add_release_to_collage(config: &Config, collage_name: &str, release_id: &str) -> Result<()> {
    ...
    for r in &data.releases {
        if r.uuid == release_id {
            unlock(&conn, &collage_lock_name(collage_name))?;
            info!(
                "No-Op: Release {} already in collage {}",
                release_logtext, collage_name
            );
            return Ok(());
        }
...
pub fn edit_collage_in_editor(config: &Config, collage_name: &str) -> Result<()> {
    ...
    let temp_file = config
        .cache_dir
        .join(format!("rose-edit-collage-{}.txt", collage_name));
    ...
    // Create UUID mapping
    let uuid_mapping: HashMap<String, String> = data
        .releases
        .iter()
    ...
    for desc in edited_content.trim().split('\n') {
        ...
        let uuid = uuid_mapping.get(desc).ok_or_else(|| {
            RoseError::Expected(RoseExpectedError::Generic(format!(
                "Release {} does not match a known release in the collage. Was the line edited?",
                desc
            )))
        })?;
