...
fn test_split_tag() {
    let result = split_tag(Some(
        "Artist A \\\\ Artist B / Artist C; Artist D vs. Artist E",
    ));
    assert_eq!(
        result,
        vec!["Artist A", "Artist B", "Artist C", "Artist D", "Artist E"]
    );
...
fn test_format_artist_string() {
    ...
    assert_eq!(
        ...
        "Artist A produced by Producer A"
    );
