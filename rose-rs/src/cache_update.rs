...
use crate::cache::{
    cached_release_from_view, cached_track_from_view, connect, CachedRelease, Release, Track,
...
pub fn update_cache_evict_nonexistent_releases(config: &Config) -> Result<()> {
    ...
    let params: Vec<&dyn rusqlite::ToSql> =
        dir_strs.iter().map(|s| s as &dyn rusqlite::ToSql).collect();
...
pub fn update_cache_for_releases(
    ...
    debug!(
        "Refreshing the read cache for {} releases",
        release_dirs.len()
    );
    ...
    if release_dirs.len() < 50 {
        debug!(
            "Running cache update executor in same process because len={} < 50",
            release_dirs.len()
        );
...
fn _update_cache_for_releases_executor_with_receivers(
    ...
    release_dirs: Vec<PathBuf>,
    _force: bool,
    _collages_to_force_update_receiver: Arc<Mutex<Vec<String>>>,
    _playlists_to_force_update_receiver: Arc<Mutex<Vec<String>>>,
) -> Result<()> {
    ...
    debug!(
        "Release update source dir scan time {:?}",
        dir_scan_start.elapsed()
    );
...

    {
        ...
        if !release_uuids.is_empty() {
            ...
            let sql = format!("SELECT * FROM releases_view WHERE id IN ({})", placeholders);
    ...
    debug!(
        "Release update cache read time {:?}",
        cache_read_start.elapsed()
    );

    // Now iterate over all releases in the source directory
    let loop_start = Instant::now();
    let mut _upd_delete_source_paths: Vec<String> = Vec::new();
    let mut _upd_release_args: Vec<Vec<Box<dyn rusqlite::ToSql>>> = Vec::new();
    let mut _upd_release_ids: Vec<String> = Vec::new();
    let mut _upd_release_artist_args: Vec<Vec<Box<dyn rusqlite::ToSql>>> = Vec::new();
    let mut _upd_release_genre_args: Vec<Vec<Box<dyn rusqlite::ToSql>>> = Vec::new();
    let mut _upd_release_secondary_genre_args: Vec<Vec<Box<dyn rusqlite::ToSql>>> = Vec::new();
    let mut _upd_release_descriptor_args: Vec<Vec<Box<dyn rusqlite::ToSql>>> = Vec::new();
    let mut _upd_release_label_args: Vec<Vec<Box<dyn rusqlite::ToSql>>> = Vec::new();
    let mut _upd_unknown_cached_tracks_args: Vec<(String, Vec<String>)> = Vec::new();
    let mut _upd_track_args: Vec<Vec<Box<dyn rusqlite::ToSql>>> = Vec::new();
    let mut _upd_track_ids: Vec<String> = Vec::new();
    let mut _upd_track_artist_args: Vec<Vec<Box<dyn rusqlite::ToSql>>> = Vec::new();
    ...
    debug!(
        "Release update scheduling loop time {:?}",
        loop_start.elapsed()
    );
