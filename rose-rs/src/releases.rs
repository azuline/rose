...
use crate::cache::{
    connect, get_release, get_tracks_of_release, list_releases, lock, unlock,
    release_lock_name, CachedRelease, CachedTrack, STORED_DATA_FILE_REGEX,
};
use crate::cache_update::{
    update_cache_evict_nonexistent_releases, update_cache_for_collages,
    update_cache_for_playlists, update_cache_for_releases,
};
use crate::common::{Artist, ArtistMapping};
use crate::config::Config;
use crate::datafiles::{
    datafile_path, find_release_datafile, read_datafile, write_datafile, 
    StoredDataFile,
};
...
use crate::templates::evaluate_release_template;
use chrono::Utc;
use regex::Regex;
...
pub fn toggle_release_new(config: &Config, release_id: &str) -> Result<()> {
    ...
    let datafile_path = release.source_path.join(format!(".rose.{}.toml", release_id));
...
pub fn create_release(
    ...
    source_dir: &Path,
    title: &str,
    artists: Vec<(String, String)>, // (name, role)
) -> Result<String> {
...
pub fn edit_release(
    ...
    artists_map.insert("main".to_string(), 
        release.releaseartists.main.iter().map(|a| a.name.clone()).collect());
    artists_map.insert("guest".to_string(), 
        release.releaseartists.guest.iter().map(|a| a.name.clone()).collect());
    artists_map.insert("remixer".to_string(), 
        release.releaseartists.remixer.iter().map(|a| a.name.clone()).collect());
    artists_map.insert("producer".to_string(), 
        release.releaseartists.producer.iter().map(|a| a.name.clone()).collect());
    artists_map.insert("composer".to_string(), 
        release.releaseartists.composer.iter().map(|a| a.name.clone()).collect());
    artists_map.insert("conductor".to_string(), 
        release.releaseartists.conductor.iter().map(|a| a.name.clone()).collect());
    artists_map.insert("djmixer".to_string(), 
        release.releaseartists.djmixer.iter().map(|a| a.name.clone()).collect());
    ...
    let editable = EditableRelease {
        ...
        release_year: release.releasedate.as_ref().map(|d| d.year as i32),
        original_year: release.originaldate.as_ref().map(|d| d.year as i32),
        composition_year: release.compositiondate.as_ref().map(|d| d.year as i32),
    ...
    let temp_file = config.cache_dir.join(format!("rose-edit-{}.toml", release_id));
...
fn create_edit_actions(original: &CachedRelease, edited: &EditableRelease) -> Vec<Action> {
    ...
    if original.releasetitle != edited.title {
        actions.push(Action {
            ...
            behavior: crate::rule_parser::ActionBehavior::Replace(
                crate::rule_parser::ReplaceAction {
...
pub fn set_release_cover_art(
    ...
    if !cover_art_path.exists() {
        return Err(RoseError::Expected(RoseExpectedError::Generic(
            format!("Cover art file does not exist: {:?}", cover_art_path)
        )));
    }
    ...
    info!("Set cover art for release {} to {}", release.releasetitle, dest_filename);
...
pub fn delete_release_cover_art(config: &Config, release_id: &str) -> Result<()> {
    ...
    if release.cover_image_path.is_none() {
        return Err(RoseError::Expected(RoseExpectedError::Generic(
            format!("Release {} has no cover art", release_id)
        )));
    }
...
pub fn create_single_release(
    ...
    info!("Created single release {} from track {}", single_title, track_id);
