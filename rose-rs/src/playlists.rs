...
pub fn remove_track_from_playlist(
    ...
    let track_logtext = get_track_logtext(config, track_id)?.ok_or_else(|| {
    ...
    if old_len == data.tracks.len() {
        ...
        info!(
            "No-Op: Track {} not in playlist {}",
            track_logtext, playlist_name
        );
        return Ok(());
    }
...
pub fn add_track_to_playlist(config: &Config, playlist_name: &str, track_id: &str) -> Result<()> {
    ...
    for t in &data.tracks {
        if t.uuid == track_id {
            unlock(&conn, &playlist_lock_name(playlist_name))?;
            info!(
                "No-Op: Track {} already in playlist {}",
                track.tracktitle, playlist_name
            );
            return Ok(());
        }
    }
...
pub fn edit_playlist_in_editor(config: &Config, playlist_name: &str) -> Result<()> {
    ...
    for track in &data.tracks {
        *line_occurrences
            .entry(track.description_meta.clone())
            .or_insert(0) += 1;
    }
    ...
    let temp_file = config
        .cache_dir
        .join(format!("rose-edit-playlist-{}.txt", playlist_name));
    ...
    for desc in edited_content.trim().split('\n') {
        ...
        let uuid = uuid_mapping.get(desc).ok_or_else(|| {
            RoseError::Expected(RoseExpectedError::Generic(format!(
                "Track {} does not match a known track in the playlist. Was the line edited?",
                desc
            )))
        })?;
...
pub fn set_playlist_cover_art(
            ...
            if stem == playlist_name {
                if let Some(ext) = file_path.extension() {
                    if config
                        .valid_art_exts
                        .iter()
                        .any(|valid_ext| valid_ext.eq_ignore_ascii_case(&ext.to_string_lossy()))
                    {
    ...
    info!(
        "Set the cover of playlist {} to {:?}",
        playlist_name,
        cover_art_path.file_name().unwrap()
    );
