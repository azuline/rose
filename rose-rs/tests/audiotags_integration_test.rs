...
fn test_getters_track1_flac() {
    ...
    assert_eq!(af.releasedate, Some(RoseDate { year: 1990, month: None, day: None }));
    // Note: originaldate is in custom tags which may not be present
    assert_eq!(af.originaldate, Some(RoseDate { year: 1990, month: None, day: None }));
    assert_eq!(af.compositiondate, Some(RoseDate { year: 1984, month: None, day: None }));
    ...
    assert_eq!(
        ...
        vec![Artist { name: "Artist A".to_string(), alias: false }, Artist { name: "Artist B".to_string(), alias: false }]
    );
    assert_eq!(
        af.trackartists.guest,
        vec![Artist { name: "Artist C".to_string(), alias: false }, Artist { name: "Artist D".to_string(), alias: false }]
    );
...
fn test_flush_track1_flac() {
    ...
    af.trackartists.djmixer = vec![Artist { name: "New".to_string(), alias: false }];
    // Also test date writing
    af.originaldate = Some(RoseDate { year: 1990, month: Some(4), day: Some(20) });
    ...
    assert_eq!(af.releasetitle, Some("A Cool Album".to_string()));
    assert_eq!(af.releasetype, "album");
    assert_eq!(af.releasedate, Some(RoseDate { year: 1990, month: Some(2), day: Some(5) }));
    assert_eq!(af.originaldate, Some(RoseDate { year: 1990, month: Some(4), day: Some(20) }));
    assert_eq!(af.compositiondate, Some(RoseDate { year: 1984, month: None, day: None }));
    ...
    assert_eq!(af.catalognumber, Some("DN-420".to_string()));
    assert_eq!(af.edition, Some("Japan".to_string()));
    assert_eq!(af.trackartists.djmixer, vec![Artist { name: "New".to_string(), alias: false }]);
    assert_eq!(af.duration_sec, 2);
}
